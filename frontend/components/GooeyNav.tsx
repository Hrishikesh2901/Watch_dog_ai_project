...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>, index: number) => {
    e.preventDefault(); // Prevent default anchor jump, using onClick handler
    
    // Call custom onClick if provided (e.g. for smooth scroll)
    if (items[index].onClick) {
        items[index].onClick!();
    } else if (items[index].href) {
        // Fallback for direct links
        if (items[index].href.startsWith('#')) {
             const id = items[index].href.substring(1);
